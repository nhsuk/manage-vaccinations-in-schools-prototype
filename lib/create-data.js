...
// Cohorts
let cohorts = []
for (const yearGroup of [...range(0, 11)]) {
  const uploadedRecords = records.filter((record) => !record._pending)

  const cohort = Cohort.generate(uploadedRecords, yearGroup)
...
for (const [urn, patients] of Object.entries(children)) {
  ...
  for (const programme of programmes) {
    ...
    for (const patient of patients) {
      ...
      const patientInvitedToSession = patient.sessions.includes(session.id)
