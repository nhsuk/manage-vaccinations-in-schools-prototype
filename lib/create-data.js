...
import { Programme } from '../app/models/programme.js'
...
for (const preset of Object.values(SessionPresets)) {
  ...
  for (const school_urn of urns) {
    const schoolSession = generateSession(preset, year, nurse, { school_urn })
    if (schoolSession) {
...
for (const patient of Object.values(context.patients)) {
  if (patient?.pendingChanges?.school_urn) {
    const move = new Move({
      ...
      from_code:
        matchingIndex === 0
          ? Object.values(context.organisations)[0].code
          : undefined,
