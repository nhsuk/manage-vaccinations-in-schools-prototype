...
// Programmes
context.programmes = {}
const programmes = Object.values(programmesData).filter(
...
for (const preset of Object.values(SessionPresets)) {
  ...
  const ids = Object.values(context.clinics).flatMap(({ id }) => id)

  // Schedule clinic sessions
  for (const clinic_id of ids) {
...
// Ensure at least one session is scheduled for today
const earliestPlannedSession = Object.values(context.sessions)
  .map((session) => new Session(session))
  .sort((a, b) => getDateValueDifference(a.openAt, b.openAt))
  .find((session) => session.isPlanned)

const hasSessionToday = isSameDay(earliestPlannedSession?.date, today())

if (!hasSessionToday) {
  context.sessions[earliestPlannedSession.id].date = today()
}
...
for (let session of Object.values(context.sessions)) {
  ...
  const patientsInClinic = Object.values(context.patients).filter(
    ({ clinic_id }) => clinic_id === session.clinic_id
  )

  for (let patient of patientsInClinic) {
