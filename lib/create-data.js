...
for (const programme of programmes.values()) {
  for (const yearGroup of programme.yearGroups) {
    let cohort = Cohort.generate(programme, records, yearGroup, nurse)
...
for (const cohort of cohorts.values()) {
  const record_nhsns = cohort.record_nhsns.map((nhsn) => records.get(nhsn))

  programme = programmes.get(cohort.programme_pid)

  const cohortImport = Import.generate(programme, record_nhsns, nurse)
  imports.set(cohortImport.id, cohortImport)
  const schoolImport = Import.generate(
    programme,
    record_nhsns.slice(0, 30),
    nurse,
    ImportType.School
  )
...
for (const patient of patients.values()) {
  const school = School.generate(patient.record.urn)

  schools.set(school.urn, school)
}

// Add unknown school and home-schooled URNs
schools.set(888888, School.generate(888888))
schools.set(999999, School.generate(999999))
...
for (const programme of programmes.values()) {
  ...
  let urns = []
  for (const uid of programme.cohort_uids) {
...
for (const patient of patients.values()) {
  ...
  const programme = [...programmes.values()].find(
    (programme) => programme.pid === schoolSession.programme_pids[0]
  )
  ...
  if (patientInSession && addRepliesToPatient) {
    ...
    Array.from([...range(0, maxReplies)]).forEach(() => {
      ...
      const reply = Reply.generate(programme, session, patient)
...
for (const programme of programmes.values()) {
  const programmeSessions = [...sessions.values()].filter((session) =>
    session.programme_pids.includes(programme.pid)
  )
