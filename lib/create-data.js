...
import { ConsentWindow, Session, SessionType } from '../app/models/session.js'
...
for (let session of sessions.values()) {
  session = new Session(sessions.get(session.id), {
    clinics: Object.fromEntries(clinics),
    patients: Object.fromEntries(patients),
    programmes: Object.fromEntries(programmes),
    schools: Object.fromEntries(schools)
  ...
  for (const patient of session.cohort) {
    for (const programme_pid of session.programme_pids) {
      ...
      // 2️⃣ INVITE parent to give consent
      const clinicSession = session.type === SessionType.Clinic
      const notInSchool = ['888888', '999999'].includes(patient.school_urn)

      if (clinicSession && notInSchool) {
        patient.inviteToSession(patientSession)
        patientSessions.set(patientSession.uuid, patientSession)
      } else if (!clinicSession && !notInSchool) {
        patient.inviteToSession(patientSession)
        patientSessions.set(patientSession.uuid, patientSession)
      }
    }
...
for (let patientSession of patientSessions.values()) {
  ...
  patientSession = new PatientSession(patientSession, {
    ...
    schools: Object.fromEntries(schools),
    sessions: Object.fromEntries(sessions),
    vaccinations: Object.fromEntries(vaccinations)
  })
  ...
  if (patientSession.screen === ScreenOutcome.NeedsTriage) {
    ...
    for (const response of patientSession.responsesWithTriageNotes) {
      ...
      if (triaged) {
        ...
        // 4️⃣ SCREEN with triage outcome
  ...
  const patient = patients.get(patientSession.patient_uuid)
  const session = sessions.get(patientSession.session_id)

  // Vaccination outcome
  if (session.isCompleted) {
    for (const programme of session.programmes) {
      if (patientSession.nextActivity === Activity.Record) {
        // REGISTER attendance
        const present = faker.datatype.boolean(0.8)
        patientSession.registerAttendance(
          {
            createdAt: session.firstDate,
            createdBy_uid: nurse.uid
          },
          present ? RegistrationOutcome.Present : RegistrationOutcome.Absent
        )
