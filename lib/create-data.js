...
import { Campaign, CampaignType } from '../app/models/campaign.js'
...
// Campaigns, cohorts and patients
let campaigns = []
let cohorts = []
let patients = []
for (const type of Object.values(CampaignType)) {
  // Create campaign
  const campaign = Campaign.generate(type, exampleUser)

  // Create campaign cohorts
  const campaignCohorts = Cohort.generateAll(campaign, records, exampleUser)
  campaign.cohorts = campaignCohorts.map((cohort) => cohort.uuid)
  cohorts.push(...campaignCohorts)

  // Use first cohort
  campaign.pendingRecords = []

  for (const cohort of campaignCohorts) {
    // Create pending records from all cohorts in campaign
    campaign.pendingRecords.push(...cohort.pendingRecords)

    // Create patient records and attach them to campaign
...
for (const [urn, patients] of Object.entries(children)) {
  ...
  for (const campaign of campaigns) {
    ...
    for (const patient of patientsInCampaign) {
      ...
      if (patientInvitedToSession && patientHasReplies) {
        ...
        // Unmatch a reply
        if (sessionPlanned && patientInvitedToSession && replyCount > 1) {
...
generateDataFile('.data/cohorts.json', _.keyBy(cohorts, 'uuid'))
