...
for (const type of Object.values(CampaignType)) {
  ...
  const { yearGroups } = campaignTypes[campaign.type]
  for (const yearGroup of yearGroups) {
    const cohort = Cohort.generate(campaign, records, exampleUser, yearGroup)
    cohorts.push(cohort)
  }
  ...
  for (const cohort of cohorts) {
    for (const nhsn of cohort.records) {
      ...
      const selectPatientForCampaign = faker.helpers.maybe(() => true, {
        probability: 0.25
      })

      // Select for campaign
      if (selectPatientForCampaign) {
        patient.select = cohort
        patients.push(patient)
      } else {
        campaign.pendingCohort.push(patient.nhsn)
      }
    }
  }
}
