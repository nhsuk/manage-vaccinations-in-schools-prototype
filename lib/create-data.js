...
import { Notice, NoticeType } from '../app/models/notice.js'
...
for (const cohort of cohorts.values()) {
  ...
  const cohortImport = Import.generate(programme, importedRecords, nurse)
  imports.set(cohortImport.id, cohortImport)
  const schoolImport = Import.generate(
    programme,
    importedRecords.slice(0, 30),
    nurse,
    ImportType.School
  )
...
for (const cohort of cohorts.values()) {
  for (const nhsn of cohort.records) {
    ...
    if (patient) {
      // Temporarily add context for use during data creation
      patient = new Patient(patient, { records: Object.fromEntries(records) })
...
// Add unknown school and home-schooled URNs
schools.set(888888, new School(schoolsData[888888]))
schools.set(999999, new School(schoolsData[999999]))
...
for (const programme of programmes.values()) {
  ...
  for (const urn of urns) {
    ...
    // Schedule school session
    // Don’t create sessions for home-schooled and school unknown
    if (urn !== 888888 && urn !== 999999) {
      // Primary and secondary school to schedule sessions at today
      const isToday = urn === 139383 || urn === 135335

      const schoolSession = Session.generate(urn, programme, nurse, { isToday })
      sessions.set(
        schoolSession.id,
        new Session(schoolSession, {
  ...
  for (const id of ['X99999']) {
    const firstDate = addDays(getToday(), 30)
    const secondDate = addDays(getToday(), 60)

    // Schedule clinic session
    clinicSession = new Session({
      clinic_id: id,
      created_user_uid: nurse.uid,
...
for (const patient of patients.values()) {
  const schoolSession = [...sessions.values()].find(
  ...
  const sessionCompleted = schoolSession.status === SessionStatus.Completed
  const sessionPlanned = schoolSession.status === SessionStatus.Planned
  const sessionUnplanned = schoolSession.status === SessionStatus.Unplanned

  // TODO: Support sessions with multiple programmes
  const programme = [...programmes.values()].find(
    (programme) => programme.pid === schoolSession.programmes[0]
  )

  // Home-schooled and school unknown children get directly invited to clinics
  const session =
    patient.record.urn === 888888 || patient.record.urn === 999999
      ? clinicSession
      : schoolSession

  // 2️⃣ INVITE patient to (school or clinic) session
...
const deceasedNotice = Notice.generate(deceasedPatient, NoticeType.Deceased)
notices.set(deceasedNotice.uuid, deceasedNotice)
deceasedPatient.flag = deceasedNotice

// Remove patient from all cohorts
for (const uid of deceasedPatient.cohorts) {
...
if (deceasedPatient.session_id) {
  const session = new Session({
    ...sessions.get(deceasedPatient.session_id),
    ...{ created: getToday(2) },
    ...{ created_user_uid: false }
  })

  deceasedPatient.disinvite = session
}

// Flag patient record as invalid
const invalidPatient = [...patients.values()][1]
const invalidNotice = Notice.generate(invalidPatient, NoticeType.Invalid)
notices.set(invalidNotice.uuid, invalidNotice)
invalidPatient.flag = invalidNotice

// Flag patient record as sensitive
const sensitivePatient = [...patients.values()][2]
const sensitiveNotice = Notice.generate(sensitivePatient, NoticeType.Sensitive)
notices.set(sensitiveNotice.uuid, sensitiveNotice)
sensitivePatient.flag = sensitiveNotice
...
const hiddenNotice = Notice.generate(vaccinatedPatient, NoticeType.Hidden)
notices.set(hiddenNotice.uuid, hiddenNotice)
vaccinatedPatient.flag = hiddenNotice
