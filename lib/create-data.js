...
// Add cohort upload
const patient_nhsns = Object.values(context.patients).flatMap(
  ({ nhsn }) => nhsn
)
const cohortUpload = generateUpload(patient_nhsns, nurse)
...
for (const school of Object.values(context.schools)) {
  const patient_nhsns = Object.values(context.patients)
...
for (const programme of Object.values(context.programmes)) {
  ...
  programmeVaccinations.forEach(({ patientSession_uuid }) => {
    ...
    if (hasPatientSession) {
      ...
      patient_nhsns.push(patientSession.patient.nhsn)
    }
  })
...
const vaccinatedPatient = Object.values(context.patients).find(
...
// Flag patient record as gillick competent, no parent notification
const gillickPatient = Object.values(context.patients)[40]
if (gillickPatient) {
  // Add vaccination record
  const vaccinationRecord = new Vaccination({
    // outcome: VaccinationOutcome.Vaccinated,
    programme_id: ProgrammeType.Flu
  })
  gillickPatient.recordVaccination(vaccinationRecord)

  // Add self consent
  const gillickReply = new Reply(
    {
      child: gillickPatient,
      programme_id: ProgrammeType.Flu,
      selfConsent: true
    },
    context
  ...
  gillickPatient.addReply(gillickReply)

  const gillickNotice = generateNotice(
    gillickPatient,
    NoticeType.GillickNoNotify
  )
  context.notices[gillickNotice.uuid] = gillickNotice
  gillickPatient.addNotice(gillickNotice)
}
