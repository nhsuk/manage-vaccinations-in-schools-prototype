...
// Add cohort upload
const patient_nhsns = [...patients.values()].flatMap(({ nhsn }) => nhsn)
...
for (const school of schools.values()) {
  const patient_nhsns = [...patients.values()]
...
for (const [programmePreset, preset] of Object.entries(ProgrammePreset)) {
  ...
  for (const year of Object.values(AcademicYear)) {
    for (const school_urn of urns) {
      let schoolSession = generateSession(programmePreset, year, nurse, {
        school_urn
      })
      if (schoolSession) {
  ...
  for (const year of Object.values(AcademicYear)) {
    for (const clinic_id of ['X99999']) {
      clinicSession = generateSession(programmePreset, year, nurse, {
        clinic_id
      })
      if (clinicSession) {
