...
import { School, SchoolPhase, schoolTerms } from '../app/models/school.js'
import {
  ConsentWindow,
  Session,
  SessionStatus,
...
for (const school of Object.values(schoolsData)) {
  schools.set(school.urn, School.generate(school.urn))
}
...
for (const cohort of cohorts.values()) {
  ...
  const cohortImport = Import.generate(programme, cohort.record_nhsns, nurse)
...
for (const [term, programmes] of Object.entries(programmesGroupedByTerm)) {
  ...
  for (const school_urn of urns) {
    const schoolSession = Session.generate(pids, schoolTerms[term], nurse, {
      school_urn
    })
    sessions.set(schoolSession.id, new Session(schoolSession))
    ...
    const schoolPatients = [...patients.values()].filter(
      ({ record }) => record.school_urn === school_urn
    )
  ...
  for (const clinic_id of ['X99999']) {
    clinicSession = Session.generate(pids, schoolTerms[term], nurse, {
      clinic_id
    })
    sessions.set(clinicSession.id, new Session(clinicSession))
  }

  const clinicPatients = [...patients.values()].filter(({ record }) =>
    [888888, 999999].includes(record.school_urn)
  )
...
for (const patient of patients.values()) {
  ...
  for (const session of patientSessions) {
    ...
    for (const programme of sessionProgrammes) {
      ...
      // Vaccination outcome
      if (sessionCompleted) {
        const school = schools.get(patient.record.school_urn)
...
for (const record of records.values()) {
  if (record?.pendingChanges?.school_urn) {
    const move = new Move({
      from: record.school_urn,
      to: record?.pendingChanges?.school_urn,
