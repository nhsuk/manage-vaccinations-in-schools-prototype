...
for (const cohort of cohorts.values()) {
  ...
  const cohortImport = Import.generate(programme, cohort.patient_nhsns, nurse)
  imports.set(cohortImport.id, cohortImport)
  const schoolImport = Import.generate(
    programme,
    cohort.patient_nhsns.slice(0, 30),
    nurse,
    ImportType.School
  )
...
for (const cohort of cohorts.values()) {
  for (const nhsn of cohort.patient_nhsns) {
...
vaccinations.forEach((vaccination) => {
  ...
  vaccinationRecords.push(new Record(patient))
})
