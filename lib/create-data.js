    ...
    for (const programme of sessionProgrammes) {
      ...
      if (addRepliesToPatient) {
        ...
        Array.from([...range(0, maxReplies)]).forEach(() => {
          ...
          replies.set(reply.uuid, reply)
        })
      }

      // Unmatch a reply
      const patientReplies = Object.values(patient.replies)
      if (sessionPlanned && patientReplies.length > 2) {
        const replyToUnmatch = patientReplies[2]
        ...
        // Add reply to unmatched consents
        consents.set(replyToUnmatch.uuid, replyToUnmatch)

        // Remove reply from matched consents
        replies.delete(replyToUnmatch.uuid)

        // Remove reply from patient record
        patient.reply_uuids = patient.reply_uuids.filter(
          (uuid) => uuid !== replyToUnmatch.uuid
        )
      }
