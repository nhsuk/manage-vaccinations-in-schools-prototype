...
import { Import } from '../app/models/import.js'
...
for (const cohort of cohorts.values()) {
  const importedRecords = cohort.records
    .map((nhsn) => records.get(nhsn))
    .filter((record) => !record._pending)

  const _import = Import.generate(cohort.programme_pid, importedRecords, nurse)
...
for (const cohort of cohorts.values()) {
  for (const nhsn of cohort.records) {
    ...
    if (!patient) {
      ...
      // Ignore records that have not yet been imported
      if (!record._pending) {
...
for (const patient of patients.values()) {
  const attendedSchool = schoolsData[patient.record.urn]

  // Map GIAS field names to Mavis field names
  const school = new School({
    urn: attendedSchool.urn,
    name: attendedSchool.name,
    phase: attendedSchool.phase,
    addressLine1: attendedSchool.address_line1,
    addressLine2: attendedSchool.address_line2,
    addressLevel1: attendedSchool.address_level1,
    postalCode: attendedSchool.postal_code
  })
...
for (const patient of patients.values()) {
  ...
  if (patientInSession && sessionCompleted) {
    ...
    if (process.env.FEATURE_PENDING_VACCINATIONS) {
      // Mark vaccination as pending; requires import to see on CHIS
