...
export class Patient extends Record {
   ...
   * @param {import('./patient-session.js').PatientSession} patientSession - Patient session
   */
  removeFromSession(patientSession) {
    this.patientSession_uuids = this.patientSession_uuids.filter(
      (uuid) => uuid !== patientSession.session.id
    )
    this.addEvent({
      type: EventType.Select,
      name: `Removed from the ${patientSession.session.name}`,
      createdAt: patientSession.session.createdAt,
      createdBy_uid: patientSession.session.createdBy_uid
    })
  }
